...
async function test() {
  try {
    const count = await executeKw('account.move.line', 'search_count', [
      [
        ['account_id', 'in', [81]],
        ['parent_state', '=', 'posted']
      ]
    ...
    console.log('Total Posted lines for Produce (200110):', count);
    
    const countAll = await executeKw('account.move.line', 'search_count', [
      [
        ['account_id', 'in', [81, 82, 1270]],
        ['parent_state', '=', 'posted']
      ]
    ]);
    
    console.log('Total Posted lines for all 3 accounts:', countAll);
